...
def get_vap_stats(e):
    vap_stats = {}
    try:
        vap_status = sp.check_output('hostapd_cli -p ' + ctrl_dir + e + ' status', universal_newlines=True,
                                     shell=True).split('\n')
...
def get_sta_stats(e):
    ...
    num_stas = 0
    try:
        all_sta = sp.check_output('hostapd_cli -p ' + ctrl_dir + e + ' all_sta', universal_newlines=True,
                                  shell=True).split('\n')
    except Exception as e:
                ...
                for metric in vap_stats.keys():
                    if (metric in metrics_ap):
                        if (metrics_ap[metric][1] == 'gauge'):
                            g = GaugeMetricFamily('hostapd_ap_' + metrics_ap[metric][0], metrics_ap[metric][2],
                                                  labels=['id'])
                        else:
                            g = CounterMetricFamily('hostapd_ap_' + metrics_ap[metric][0], metrics_ap[metric][2],
                                                    labels=['id'])
                    ...
                    for metric in sta.keys():
                        if (metric in metrics_sta):
                            if (metrics_sta[metric][1] == 'gauge'):
                                g = GaugeMetricFamily('hostapd_sta_' + metrics_sta[metric][0], metrics_sta[metric][2],
                                                      labels=['id', 'mac_sta'])
                            else:
                                g = CounterMetricFamily('hostapd_sta_' + metrics_sta[metric][0],
                                                        metrics_sta[metric][2], labels=['id', 'mac_sta'])
...
def main():
    ...
    if (len(sys.argv) > 1):
        try:
            ...
            print(
                "Exception while opening the config.json file: Add it to the directory or use its location as an argument")
            exit(-1)
    else:
        try:
